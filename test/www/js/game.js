                    ...
                    if (rs.rows.length > 0) {
                        ...
                        if (isMaterialDiscovered && isCreditInRange && isLikeInRange) { // qui si accede se le condizioni sono raggiunte
                            ...
                            db.executeSql(query_new_materials, [questId], (rsMaterials) => {
                                ...
                                // resetto anche qui le categorie, per adeguarmi alle nuove molecole scoperte
...
function endGame(slot) { // renderizza le statistiche di gioco a chiusura, che recupero da localstorage
    const game_data = JSON.parse(localStorage.getItem(`Slot${slot}`));
    // mi serve anche la descrizione del livello, per maggior divertimento
    const level_desc_query = `SELECT description from levels WHERE id = ?`;
    window.sqlitePlugin.openDatabase({name: "../assets/chimgio.db", location: "default"}, (db) => {
        db.executeSql(level_desc_query, [game_data.level], (res) => {
            if (res.rows.length > 0) {
                const description = res.rows.item(0).description;
            }
        });
    });
    let modal = document.querySelector("#laboratorio #endgame");
    modal.innerHTML = `
    <div>
        <p>STATISTICHE DI FINE GIOCO:</p>
        <ol>
            <li>Livello ${game_data.level} (${description})</li>
            <li>Esperienza ${game_data.experience}</li>
            <li>Credito ${game_data.credit}</li>
            <li>Like ricevuti ${game_data.like}</li>
            <li>Numero di combinazioni effettuate ${game_data.combination_done_list.length}</li>
            <li>Numero di sostanze scoperte ${game_data.material_discovered_list.length}</li>
            <li>Numero di quest completate ${game_data.quest_done_list.length}</li>
        </ol>
    </div>`;
    // cancello i dati nello slot corrente
    localStorage.setItem(`Slot${game_data.slot}`, null);
    // alla fine ritorno alla home con un click
    setTimeout(() => {
        modal.addEventListener("click", function() {
            modal.hide();
            modal.innerHTML = "";
